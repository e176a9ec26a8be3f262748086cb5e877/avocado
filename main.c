...
int main() {
    ...
    const char *emulator_path = "/home/user/Android/Sdk/emulator/emulator";
    const char *adb_path = "/home/user/Android/Sdk/platform-tools/adb";
    ...
    snprintf(new_path, sizeof(new_path), "%s/bin:%s:%s:%s/platform-tools:%s/tools/bin:%s:%s",
             java_home, sdk_path, sdk_path, "/home/user/Android/Sdk", "/home/user/Android/Sdk", "/home/user/Android/Sdk", current_path);
    ...
    while (1) {
        ...
        if (strncmp(command, "emulator", 8) == 0) {
            snprintf(emulator_command, sizeof(emulator_command), "%s %s", emulator_path, command + 9);
        ...
        else if (strncmp(command, "adb", 3) == 0) {
            snprintf(emulator_command, sizeof(emulator_command), "%s %s", adb_path, command + 4);

            int status = system(emulator_command);
            if (status == -1) {
                perror("Erro ao executar comando do adb");
            }
        }
        else {
